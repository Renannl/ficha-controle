...
import { X, Upload, FileSpreadsheet, Trash2, FileText, Eye, Download } from "lucide-react";
...
const LABEL_PARA_TIPO = Object.fromEntries(
  Object.entries(PAINEL_LABELS).map(([valor, label]) => [label.trim().toLowerCase(), valor])
);
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const linhasMatriz = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
        ...
        const idxCliente = headerColecao.findIndex((h) => String(h).trim().toLowerCase() === "cliente");
        const idxObservacao = headerColecao.findIndex((h) => String(h).trim().toLowerCase() === "observação");

        const clienteBase = idxCliente !== -1 ? String(valoresColecao[idxCliente] || "").trim() : "";
        const observacaoColecao = idxObservacao !== -1 ? String(valoresColecao[idxObservacao] || "").trim() : "";

        if (!clienteBase) throw new Error("Cliente (linha 2) não pode estar vazio.");

        const headerIndex = linhasMatriz.findIndex((linha) =>
          linha.some((cel) => String(cel).trim().toLowerCase() === "nome equipamento")
        );
        if (headerIndex === -1) throw new Error("Cabeçalho 'Nome Equipamento' não encontrado.");
        ...
        if (linhasObjeto.length === 0) throw new Error("Nenhuma linha de equipamento.");
...
// 🆕 Função pra abrir arquivo autenticado
async function abrirArquivoAutenticado(arquivoId, nome, authFetch) {
  try {
    ...
    // Libera o blob URL depois de um tempo
...
export default function CompletarColecaoModal({ show, colecaoId, onClose, onCompletado }) {
  ...
  async function handleCompletar() {
    ...
    try {
      ...
      const res = await authFetch(`${API_URL}/colecoes/${colecaoId}/completar`, {
        method: "POST",
      ...
      onClose?.();
      alert(`Coleção completada! ${resultado.fichas?.length ?? 0} fichas criadas.`);
    } catch (err) {
    ...
    <div className="importar-colecao-overlay" onClick={onClose}>
      <div className="importar-colecao-modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="rascunho-arquivos-section">
            ...
            {carregandoArquivos ? (
              ...
              <div className="rascunho-arquivos-lista">
                {Object.entries(arquivosRascunho.arquivos || {}).map(([tipo, arquivos]) => {
                  if (!arquivos || arquivos.length === 0) return null;
                  return (
                    <div key={tipo} className="rascunho-tipo-grupo">
                      <div className="rascunho-tipo-label">{TIPOS_LABEL[tipo] || tipo}</div>
                      {arquivos.map((arq) => (
                        <button
                          key={arq.id}
                          type="button"
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            abrirArquivoAutenticado(arq.id, arq.nome, authFetch);
                          }}
                        >
                          <FileText size={14} />
                          <span className="rascunho-arquivo-nome">{arq.nome}</span>
                          <Eye size={12} className="rascunho-arquivo-view" title="Visualizar" />
                          <Download size={12} className="rascunho-arquivo-download" title="Baixar" />
                        </button>
                      ))}
                    </div>
                  );
        ...
        <div className="importar-colecao-footer">
          ...
          <button className="btn btn-primary" onClick={handleCompletar} disabled={carregando}>
