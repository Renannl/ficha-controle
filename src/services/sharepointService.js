...
export async function exportFicha(ficha, elementId = 'print-view-root') {
  ...
  if (!originalElement) {
    console.error('[Export] Elemento para impressão não encontrado:', elementId)
    return false
  }
  ...
  const timeoutId = setTimeout(() => {
    if (document.body.contains(overlay)) {
      ...
      alert('A geração demorou demais. O celular pode estar com pouca memória livre. Tente remover algumas fotos ou fechar outros aplicativos.');
    }
...
async function sendToSharePoint(ficha, pdfBlob) {
  try {
    ...
    if (response.ok) {
      console.log('[Export] Sucesso ao enviar para SharePoint via Power Automate')
    } else {
